...
class SupplierInvoice extends Page implements HasActions, HasSchemas, HasTable
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                TextColumn::make('name')->label('Supplier Name')->sortable()->searchable(),
                TextColumn::make('total_bookings')->label('Total Bookings')->sortable(),
                TextColumn::make('total_amount')->label('Total Amount')->money('GBP')->sortable(),
                TextColumn::make('supplier_cost')->label('Supplier Cost')->money('GBP')->sortable(),
                TextColumn::make('payable_amount')
                    ->label('Payable Amount')
                    ->money('GBP')
                    ->getStateUsing(fn($record) => (float) (($record->total_amount ?? 0) - ($record->supplier_cost ?? 0)))
                    ->sortable(),
            ])
            ->filters([
